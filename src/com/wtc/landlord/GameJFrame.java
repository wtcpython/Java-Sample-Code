...
public class GameJFrame extends JFrame implements ActionListener {
    ...
    // 1索引：中间的自己
    // 2索引：右边的电脑玩家
    ArrayList<ArrayList<PokerCard>> currentList = new ArrayList<>();
    ...
    // 1索引：中间的自己
    // 2索引：右边的电脑玩家
    ArrayList<ArrayList<PokerCard>> playerList = new ArrayList<>();

    // 底牌
    ArrayList<PokerCard> lordList = new ArrayList<>();

    // 牌盒，装所有的牌
    ArrayList<PokerCard> pokerList = new ArrayList<>();
    ...
    public void initCard() {
        ...
        for (int i = 1; i <= 5; i++) {
            for (int j = 1; j <= 13; j++) {
                if ((i == 5) && (j > 2)) {
                    ...
                    PokerCard poker = new PokerCard(this, i + "-" + j, false);
        ...
        ArrayList<PokerCard> player0 = new ArrayList<>();
        ArrayList<PokerCard> player1 = new ArrayList<>();
        ArrayList<PokerCard> player2 = new ArrayList<>();

        for (int i = 0; i < pokerList.size(); i++) {
            // 获取当前遍历的牌
            PokerCard poker = pokerList.get(i);
    ...
    private void initGame() {
        // 创建三个集合用来装三个玩家准备要出的牌
        for (int i = 0; i < 3; i++) {
            ArrayList<PokerCard> list = new ArrayList<>();
            // 添加到大集合中方便管理
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == landlord[0]) {
            ...
            // 获取中自己手上所有的牌
            ArrayList<PokerCard> player2 = playerList.get(1);

            // 遍历手上的牌，把要出的牌都放到临时集合中
            for (int i = 0; i < player2.size(); i++) {
                PokerCard poker = player2.get(i);
                if (poker.isClicked()) {
                    c.add(poker);
                }
            }
            ...
            if (time[0].getText().equals("不要") && time[2].getText().equals("不要")) {
                if (Common.judgeType(c) != PokerType.UnablePutCard) {
                    flag = 1;
                }
            } else {
